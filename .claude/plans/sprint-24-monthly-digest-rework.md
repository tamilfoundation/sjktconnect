...
| 1a | **News triage relevance fix — stop off-topic articles reaching `approved`** | `backend/newswatch/services/analyser.py` (or wherever Gemini relevance scoring lives), `backend/newswatch/management/commands/analyse_news_articles.py`, `backend/newswatch/management/commands/reclassify_existing_articles.py` (new), `backend/newswatch/tests/test_relevance.py` | **Surfaced by 2026-05-03 render audit**: 4 of 5 articles in the April digest's news cards were real-estate listings — KIIP Kapar industrial factory (edgeprop.my), Kulai IOI Piccolo terrace (edgeprop.my), Mirai Residences condo (edgeprop.my), one more EdgeProp rental — all scored relevance ≥3 by Gemini and auto-approved per Sprint 2.6's auto-triage rule. The single real Tamil-school article was `SJK(T) Ladang Kulai Besar` (themalaysiapress.com). Steps: (a) **Diagnose** — run `analyse_news_articles --dry-run` against the 4 known-bad URLs and capture relevance scores + Gemini reasoning to identify whether the keyword filter, the Gemini prompt, or the auto-approve threshold is at fault. (b) **Fix** (likely combination): tighten Gemini prompt to require article *subject matter* to be a Tamil school (not merely contain "tamil" or "SJK(T)" in passing — the real-estate listings probably matched on "Tamil Bazaar" location names or "SJKT nearby" amenity bullets); raise auto-approve threshold from `≥3` to `≥4` so borderline articles route to manual review; optionally add a `domain_blocklist` (edgeprop.my, propertyguru.com.my, iproperty.com.my, mudah.my) as a pre-Gemini filter. (c) **Regression test** — `test_known_bad_april_urls` asserts the 4 captured URLs score below threshold OR are filtered pre-Gemini. (d) **Backfill** — new one-shot `reclassify_existing_articles --since 2026-04-01 --status APPROVED` command reruns the new logic over April's 99 approved articles and bulk-rejects the off-topic ones. Expect 30–50% reduction. |
...
2. **v1 template — kept-and-synced vs removed.** Likely "removed" per #6 reasoning above; document the call.
...
4. **Footer CTA destinations.** Donate → `https://tamilschool.org/donate`. Forward → email-friendly share link (need to design — open question for sprint start). Document both targets.
...
6. **News triage — auto-approve threshold + domain blocklist.** S2.6 set auto-approve at relevance ≥3; April render audit shows this is too loose. Decide between threshold ≥4 (simple, may over-reject), threshold ≥3 + tightened prompt (medium effort, keeps borderline manual-review path), or threshold ≥3 + pre-Gemini domain blocklist for known classifieds sites (most surgical). Document the chosen combination and reasoning per `1a` work.
...
1. **Forward CTA target** — what URL pattern? Mailto with prefilled subject + body, or a tracked share link via Brevo's link redirector?
2. **v1 template** — really remove, or sync? (Recommend remove; need explicit OK.)
3. **Schools-by-state in v2 template** — full table (12 rows, one per state with schools) or collapsed accordion? Mobile email rendering matters here.
