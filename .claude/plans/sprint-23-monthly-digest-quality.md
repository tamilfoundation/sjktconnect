...
| 4 | **Recess detection — aggregator + prompt-wide enforcement** | `broadcasts/services/blast_aggregator.py` (working-tree change), `broadcasts/services/monthly_analyst.py` (prompt rewrite) | (a) Aggregator: `HansardSitting` filtered to `status=COMPLETED` for the digest month; if zero → `parliament_was_in_session=False`; if >0 but no SJK(T) mentions → flag separately. Closes failing `test_parliament_was_in_session_false_when_no_sittings`. (b) **Critical addition surfaced by April render audit**: the recess instruction in `ANALYST_PROMPT` lives only in the `executive_summary` section; Gemini emitted a "Parliamentary attention down" trend for April (a recess month) because `trend_lines` and `fading_from_view` instructions don't carry the recess clause. Extend the instruction across ALL prompt sections: trend_lines must NOT emit `down` for parliamentary attention when `parliament_was_in_session=False`; fading_from_view must NOT list parliamentary discourse as "fading" in a recess month; emerging_signals must skip parliamentary signals; opportunity_watch may still recommend MP outreach. (c) Verify on a re-render of April 2026 with the deployed fix that no false "down" trend for parliamentary attention appears. |
...
| 6 | **Template overhaul (v2.html)** | `backend/templates/broadcasts/monthly_blast_v2.html` | Major rework: news clusters section, schools-mentioned-by-state list, recess copy banner when `parliament_was_in_session=False`, inline brief excerpts (use `lead_paragraph` field if present), stat captions defining what each number means, click-throughs on stat numbers, three new CTA cards (Donate / Forward to a friend / See full Parliament Watch — pull through `donate_url`/`share_url`/`mp_activity_url` already in `v2_extras`), HTML entities for any em-dash/curly quotes (lesson 21). **Additions surfaced by April render audit**: (i) when `scorecards_are_lifetime_fallback=True`, suppress the scorecard section entirely OR re-frame the heading as "All-time MP champions — this month was quiet" (don't show stale lifetime data as if it's the month's news); (ii) every news/brief/meeting rendered must include a real `<a href>` to its source URL (the digest is currently a terminal node with no outbound links — converts it to a gateway into tamilschool.org); (iii) the existing footer is sparse — add unsubscribe + preferences links pulled from `subscriber.unsubscribe_token` if available; (iv) ensure the headline `by_the_numbers` row reads from Python-computed values (task #2a), not from the LLM `analysis` dict. **Not started.** |
| 6a | **NEW — Render top-N news article cards inline** | `backend/templates/broadcasts/monthly_blast_v2.html`, aggregator context | Surfaced by April render audit: the current template renders only the LLM's narrative *about* news; readers can't see the actual articles. Render the top-5 (already in `news_top` from task #2) as compact cards: title, source publication, date, link to article. Empty-state collapses gracefully when `news_total=0`. **Not started.** |
...
**File count**: ~13-17 files touched (was ~12-15 before April-render audit added task #6a + tightened #4 + #6). Still within solo budget (<20).

## April render audit (2026-05-03) — what surfaced

Running `compose_monthly_blast` against April 2026 with the currently deployed code (commit `4fa3873` — partial Sprint 23 work) revealed concrete failures that have been folded back into the task table:

- **Recess detection is structurally insufficient even after the working-tree fix lands.** The `ANALYST_PROMPT.executive_summary` instruction *does* say "If Parliament was not in session, say so explicitly rather than treating zero mentions as a signal" — but the same instruction is missing from `trend_lines`, `fading_from_view`, `emerging_signals`. Result: April render produced a "Parliamentary attention down" trend with detail "indicating a decrease in legislative focus this period." That's the exact mis-framing the executive_summary clause was designed to prevent — but only for that one section. **Rectified in task #4 above.**

- **Aggregator returns `parliament_was_in_session=None` for both April and May**, not the boolean the prompt expects. None is falsy → renders as "NOT in session (None sittings)" in the prompt. Recess-detection fix (working tree) sets this to a proper boolean for May; April logic must also set it correctly when no completed sittings exist for the entire month. Task #4 covers this.

- **Headline `by_the_numbers` (5 articles, 40 schools, 5 positive sentiment) is LLM-imputed.** Reality per Sprint 23 investigation: April had 46 approved articles. Subscribers will quote and screenshot wrong numbers. **Already in plan as task #2a — flagged again here as the single highest-trust-cost failure.**

- **Scorecards section showed "lifetime fallback" for April** (no in-month scorecard activity). Currently rendered as if it's the month's data. Task #6 now includes either suppress-or-reframe instruction.

- **Zero source links in the entire 11.9 KB rendered HTML.** The digest is a terminal node — readers cannot click through to anything. Task #6 now requires `<a href>` for every news/brief/meeting referenced.

- **News articles only appear via LLM narrative**; no actual article cards. Subscribers want to read the coverage, not just a summary OF the coverage. **NEW task #6a.**

- **CTAs absent from rendered HTML** despite `donate_url`/`share_url`/`mp_activity_url` being passed through `v2_extras` from compose command. Template just doesn't render them. Task #6 now requires the CTA card row.

- **Items deferred to Sprint 24** (separate plan): per-subscriber MP personalisation (`Subscriber.home_constituency` FK + per-recipient template injection); state-by-state slicing of schools mentioned. Both substantial; would stretch Sprint 23 from "fix digest quality" to "personalize digest". See `.claude/plans/sprint-24-personalised-digest.md`.
