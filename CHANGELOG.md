...
11 commits, one deploy chain (api-00147-vtr → api-00159-nlw; web-00182-n2d, no web rebuild needed — templates render server-side on the api service). +7 tests (346 → 357 broadcast tests). No new backend code paths outside `broadcasts/`; template + prompt + one new templatetag module.

1. **Mobile responsiveness (Gmail Android/iOS at 320-400 px)** — dropped `letter-spacing: -0.5px` + trimmed h1 to 22 px; By The Numbers rebuilt as 2×2 (was 4 columns squeezing 36 px stats into 25 %-viewport cells); `word-break: break-word` on outer container; Schools In The Spotlight table restructured from `state | schools` two-column into a stacked list so the school-name row gets full container width. Also fixed same in `parliament_watch.html`.
2. **No story reuse across analytical sections** — Gemini prompt (`broadcasts/services/monthly_analyst.py`) rewritten with a CRITICAL RULE: each section (Executive Summary, Trend Lines, Opportunity Watch, School Spotlight) must cover a *different* angle. Retired the Emerging Signals + Fading From View sections entirely — audit found both restated the top story already carried in Executive Summary. Prompt shape simplified accordingly.
3. **Analyst-top-story ↔ news-card #1 coherence** — cluster the news FIRST, pass `news_clusters` metadata into `generate_monthly_analysis()`, ask Gemini to return the cluster index it chose as the "top story," then `_promote_top_story()` pins that cluster to position 0 in the rendered news list. Guarantees the subject line + Executive Summary + first news card all name the same story.
4. **Navy brand palette** — 30 refs of `#7c3aed` (violet) → `#2563eb` (blue-600, matching tamilschool.org hero) across `monthly_blast.html`, `news_watch_urgent.html`, and `parliament_watch.html`. Owner correction: "Violet is NOT our colour. Ours is navy blue."
5. **By The Numbers + Take Action inverted to navy panel** — bg `#1e3a8a` with white numbers + light-blue (`#dbeafe`) captions. Reads as an authoritative dashboard stat bar; matches the site's blue-900 hero.
6. **Take Action buttons match site hero pattern** — 1 solid white primary (Donate — highest-priority ask for a nonprofit) + 2 outlined white-on-navy secondaries (Forward, Your MP). All three share the same border-box footprint so they line up even though only Donate has a fill. Same navy panel extends through the entire section header + button row.
7. **Whole-panel navy carries into the footer** — subscription-notice + reply-feedback line now on `#1e3a8a` with `#bfdbfe` body text. Reader's eye follows one continuous navy band down the last third of the email.
8. **Letterhead header** — 2-column top row: `SJK(T) Connect` wordmark on the left, bold month/year on the right; `Monthly Intelligence Blast` H1 spans below. Matches a printed letterhead pattern owner referenced.
9. **Canonical school-slug URLs in the newsletter** — school links in Schools In The Spotlight were still using the legacy `/school/<MOE_CODE>` shape and 301-redirecting through Cloudflare on every click. New `{% school_url s %}` template tag reuses `schools.services.revalidation.build_school_slug` (which mirrors `frontend/lib/urls.ts::schoolPath`) so links now resolve directly to `/en/school/<name>-<city>-<moe-lc>` (e.g. `/en/school/bandar-segamat-segamat-jbd7067`). 5 new tests locking the URL shape against the frontend contract.
10. **`(P### Constituency)` mentions linkified in analyst prose** — owner-flagged: analytical sections often cite MPs like `Datuk Seri Ir. Dr. Wee Ka Siong (P148 Ayer Hitam)`; the constituency page carries the MP contact card + activity, so linking adds real value. New `{% load mp_links %}` + `{{ text|linkify_mps }}` filter (regex-scans for `(P\d{3} Name)`, HTML-escapes source text, only links codes present in the `Constituency` table so hallucinated codes stay plain, locale-parameterisable). Applied to 6 free-prose fields: `executive_summary`, `trend.detail`, `opportunity_watch`, `school_spotlight.reason`, meeting-report `executive_summary`, sitting-brief `social_post_text`/`summary_html`. 6 new tests including HTML-escape safety + user's exact example string.
11. **A leaked Django `{# ... #}` multi-line comment** rendered as literal text under Schools In The Spotlight in v4/v5 previews (Django's `{# ... #}` is single-line only; the parser saw the unclosed `{#` and treated the whole block as raw text, then rendered once per for-loop iteration). Fixed by switching to `<!-- HTML comments -->` outside the loop. Recurred three separate times during this sprint — added to `docs/lessons.md`.

Live: api `sjktconnect-api-00159-nlw`, web `sjktconnect-web-00182-n2d` (unchanged; templates ship with backend).

**Aug 1 monthly blast fires automatically at 09:00 MYT with all of the above.** Preview: https://claude.ai/code/artifact/dacb5ed5-cc8c-47b1-ae95-599fe825d605 (`v10-canonical-slugs`, from Jun 2026 analyst data).
