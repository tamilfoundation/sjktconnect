...
- **Maps API key**: passed as a Docker `--build-arg NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` on `gcloud run deploy sjktconnect-web` (audit 2026-07-01 removed the baked-in ENV literal). Restricted to Maps JS, Static Maps, Places + referrer-restricted to tamilschool.org. Rotation: generate new key in Cloud Console → redeploy web with `--set-build-env-vars NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=<new>` → verify map loads → revoke old key.
