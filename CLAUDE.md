...
| 20 | **Leader Inline CRUD** | **In Progress** (started 2026-04-28 evening) — Replaces Sprint 19's read-only LeadersTab. New backend endpoints POST/PATCH/DELETE on /api/v1/schools/<moe>/leaders/ with school-admin permission gating. Frontend LeadersTab rewritten with 4 role slots, each editable (name + phone + email) or "+Add {role}" if empty. Single Save Changes button at tab footer per established pattern. School admins maintain the record (changes yearly). |
