...
| 21 | **Egress Round 2** | **In Progress** (started 2026-04-29) — Sprint 17's ISR fix didn't actually land because next-intl's `getMessages()` reads cookies/headers and marks every page as dynamic, overriding `revalidate = 86400` and forcing `Cache-Control: no-cache, no-store` upstream. Fix: `setRequestLocale(locale)` in layout + each page. Plus: block AwarioBot (~31 MB/day from one bot, unblocked); Supabase Storage hot-link protection on `school-images` bucket (~300+ MB/day from bots loading hero photos); recreate egress dashboard metrics as INT64 (DISTRIBUTION confused pickTimeSeriesFilter). |
