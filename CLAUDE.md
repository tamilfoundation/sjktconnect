...
| `accounts` | Google OAuth session + UserProfile / roles + auto-claim on @moe.edu.my (Magic-link and SchoolContact deleted in Sprint 11a) | 1.6, 11a |
...
| `BREVO_WEBHOOK_SECRET` | Yes (prod) | HMAC secret for Brevo webhook signature verification (hard-required at prod boot per audit 2026-07-01) |
...
| News Digest & Urgent Alert Fix | Done | `Broadcast.kind` + `coverage_start_date`/`coverage_end_date` (migration 0006). Digest cadence filters by `kind=NEWS_DIGEST`. Urgency classifier rewritten as two-gate + second-pass verification. `URGENT_ALERT_REQUIRE_REVIEW` feature flag added (default False at the time). 18 new tests. |
...
- **Maps API key**: still a Dockerfile literal (`frontend/Dockerfile` `ENV NEXT_PUBLIC_GOOGLE_MAPS_API_KEY=`). Sprint 33.0 attempted to move it to a build-arg but `gcloud run deploy --source . --set-build-env-vars` doesn't forward to Docker `--build-arg` for Dockerfile-based builds — only works for buildpacks. Follow-up work: add a `cloudbuild.yaml` with an explicit `docker build --build-arg` step. Meanwhile, rotation = edit the Dockerfile literal + redeploy. Restricted to Maps JS, Static Maps, Places + referrer-restricted to tamilschool.org.
...
**Reusable email-batch onboarding + Brevo quota reserve ✅ closed 2026-07-10.** Built a generic, self-cleaning pipeline to onboard any opted-in email segment via the tracked Broadcast system — replaces the fragile one-off governance scripts. New `import_email_batch` command (CSV → tagged subscribers + **all newsletter preferences created at import time**, idempotent, never resurrects unsubscribes); new `WELCOME` broadcast kind (migration `broadcasts/0009`, null coverage so multiple segments are fine); `source_tag` audience filter; `compose_welcome_broadcast` command + generic `welcome_generic.html` body (DRAFT → `send_broadcast` drip-feeds under the daily cap, tracks delivery, adds unsubscribe footer, **auto-deactivates bounces**). Also: Brevo `DEFAULT_DAILY_QUOTA` 300→275 (reserve ~25/day for utility mail). Deleted 4 superseded governance scripts. +10 tests; broadcasts+subscribers suites green (434). **Onboarding flow:** `import_email_batch --file x.csv --source-tag TAG` → `compose_welcome_broadcast --source-tag TAG` → `send_broadcast --id N`. Prod: api `sjktconnect-api-00165-9q8` carried the 275 cap; onboarding capability deployed same day.

**TF 2018 Governance welcome campaign ✅ COMPLETE 2026-07-10 (reconciled vs Brevo + prod DB).** All 537 TF_2018 subscribers sent the intro (409 delivered, 107 hard-bounced). The prior "batch 2/3 by offset" plan was WRONG (fragile offset math + incomplete Brevo CSV export) — corrected: only 54 were genuinely un-sent (all Board Chairmen, the tail cohort). Sent those 54 by explicit address; deactivated 48 hard-bounced-still-active addresses; enrolled the 131 preference-less actives into all 3 newsletters. **Do NOT re-send by offset.** Detail in `memory/tf_2018_governance_import.md`. Original import: 1,149 school leadership records + 537 newsletter subscribers, non-destructive (website data precedence), data-source attribution footer on school profile.
...
- **2026-08-01 monthly blast verify** — first live send with the Sprint 35 bundle. Watch open/click on the Take Action panel vs June baseline.
- **2026-07-17 GSC validation pull** — confirm Sprint 28 URL slug + Sprint 33.0 index adds + Cloudflare 301s shifted the numbers. Update TD-06.
- **Search Part B** (deferred from Sprint 36): after ~1 week of live search traffic, add `SearchLog` model to identify zero-result queries → owner adds targeted aliases for real pain points.
- **Owner action — janitor Cloud Run Job**: `sjktconnect-janitor-orphan-images` job + Scheduler (`0 3 * * 0` MYT) still not created (Sprint 33 mgmt cmd + `update_jobs.sh` entry both ready — "NOT FOUND (skipping)" on every deploy is expected until job is created).
- **Owner action — Maps API key rotation**: Sprint 33.0 flagged, still pending.
- **Tamil quality pass** (carryover): 87 schools carry AI-translated Tamil UNVERIFIED; owner sweep for awkward phrasing → SCHOOL_REVIEWED.
- **View-in-browser broadcast URL** (from Sprint 36 Forward-to-a-friend ask): proper "attach the newsletter" pattern needs a public broadcast-view URL. Backlog until owner pulls it in.

### Current codebase state (Sprint 36 closed, 2026-07-07)

- **Prod API**: `sjktconnect-api-00161-wx2`. Serves alias-driven search (`/api/v1/search/` queries `SchoolAlias` with ranked buckets: moe_code > canonical alias > direct name > name_tamil > weak alias), Sprint 35 template tags (`{% school_url %}`, `{{ text|linkify_mps %}}`), resume-sending 20h min-batch guard.
- **Prod web**: `sjktconnect-web-00191-mhb`. Serves map clustering (`@googlemaps/markerclusterer` wired via `useMapsLibrary("marker")` + raw `PinElement`; navy cluster bubbles scaled `sqrt(count)`), mobile-responsive InfoWindow (`width: min(280px, calc(100vw - 80px))`), schools-only search placeholder, Tamil Foundation canonical names across all locale files.
- **Tests**: 1528 backend + 366 frontend (1 test + 2 suites pre-existing failures inherited from Sprint 31/32).
- **Prod aliases table**: 4182 rows (+597 from Sprint 36 re-seed with variant map). Covers Sri/Seri, Bkt/Bukit, Tmn/Taman, Jln/Jalan, Kg/Kampung, Sg/Sungai, St/Saint, Ldg/Ladang for all 528 schools plus historical hand-curated migrations.
- **Scheduler state**: `monthly-blast` (`0 9 1 * *` MYT), `fortnightly-digest`, `urgent-alerts`, `resume-sending` — all ENABLED. Aug 1 blast auto-fires. `resume-sending` now guards against same-day double-drain via a 20h `MIN_HOURS_BETWEEN_BATCHES` check.
- **Egress alert live**: MQL 6h-bucket policy at 200MB, id `13522324934515018545`.
- **Cloudflare API access**: zone-scoped token in `.env`, perms = DNS / Zone Settings / Config Rules / Single Redirect (Edit).
- **Owner-action followups**: janitor Cloud Run Job creation, Maps API key rotation.
- **Newsletter school links**: canonical slug URLs, no 301 hop.
- **Newsletter organisation name**: `Malaysian Tamil Educational Research and Development Foundation (Tamil Foundation)` — MCEF purged 2026-07-05.
...
| 36 | **Site-quality omnibus: canonical names + map cluster + alias search** | ✅ Closed 2026-07-07 (~1.5 days, 14 commits, 3 api + 6 web deploys). Three unrelated owner-flagged threads bundled: **(a) Tamil Foundation canonical-name rename** across emails + About/Privacy/Terms in en/ms/ta. MCEF (a separate org, NOT a TF alias) purged. Trilingual canonical set saved to memory; owner-set Tamil (`பண்பாடு` not `கலாச்சாரம்`; no `மற்றும்`; short-form-only org name) + Malay (`Pertubuhan` not `Organisasi`) style rules applied to all Sprint 36 sweeps. **(b) Map clustering finally wired** — `@googlemaps/markerclusterer` was in `package.json` since Sprint 1.3, never enabled; CLAUDE.md's claim was stale. v1 (ref-callback-into-state pattern with vis.gl's `<AdvancedMarker>`) crashed the map page from a React re-render loop; reverted within 5 min. v2 (raw `AdvancedMarkerElement` + `PinElement` in single `useEffect`) shipped zero pins because removing the vis.gl JSX also removed the auto-load trigger. v2.1 (`useMapsLibrary("marker")` explicit load) works. Navy `sqrt(count)`-scaled cluster bubbles, matches brand. **(c) Alias-driven search + variant map** — SearchView rewritten with ranked buckets (moe_code > canonical alias > direct name > name_tamil > weak alias); `seed_aliases` extended with 8 Malay abbrev pairs (Sri/Seri, Bkt/Bukit, Tmn/Taman, Jln/Jalan, Kg/Kampung, Sg/Sungai, St/Saint, Ldg/Ladang). Prod re-seed: 3050 → 4182 rows. Live-verified: `?q=Sri+Alam` → JBD1029 Seri Alam; `?q=Sungai+Muar` → JBD7068 Sg Muar; `?q=Bahagian+4` → NBD4079 Bhg 4. Bonus fixes: fortnightly-digest letterhead, Forward-to-a-friend copy, mobile-responsive InfoWindow, resume-sending 20h min-batch guard. +9 tests (1519 → 1528 backend). Retro: `docs/retrospective-sprint36.md`. |
